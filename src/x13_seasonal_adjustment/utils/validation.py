...
def validate_time_series(
    ...
    if isinstance(data, pd.DataFrame):
        ...
        try:
            ...
            raise ValueError("Data must be pandas.Series, DataFrame or array-like")

    # Uzunluk kontrolü
    if len(series) < min_length:
        raise ValueError(
            f"Time series must contain at least {min_length} observations. Current: {len(series)}"
        )
    ...
    if not isinstance(series.index, pd.DatetimeIndex):
        ...
        if freq:
            series.index = pd.date_range(
                start="2000-01-01", periods=len(series), freq=freq
            )
        else:
            raise ValueError(
                "Veri DatetimeIndex'e sahip olmalıdır veya freq parametresi belirtilmelidir"
            )
    ...
    if freq:
        if series.index.freq is None:
            ...
            # Handle pandas frequency changes and aliases
            freq_mapping = {
                "M": ["ME", "M", "<MonthEnd>"],
                "ME": ["ME", "M", "<MonthEnd>"],
                "Q": ["QE", "Q", "<QuarterEnd>", "<QuarterEnd: startingMonth=12>"],
                "QE": ["QE", "Q", "<QuarterEnd>", "<QuarterEnd: startingMonth=12>"],
                "A": ["AE", "A", "<YearEnd>"],
                "AE": ["AE", "A", "<YearEnd>"],
                "Y": ["YE", "Y", "<YearEnd>"],
                "YE": ["YE", "Y", "<YearEnd>"],
            ...
            current_freq = str(series.index.freq)
            expected_aliases = freq_mapping.get(freq, [freq])

            if current_freq not in expected_aliases:
                raise ValueError(
                    f"Index frequency ({current_freq}) does not match expected frequency ({freq})"
                )
    ...
    if iqr > 0:
        ...
        if outlier_pct > 5:  # %5'ten fazla aşırı aykırı değer
            raise ValueError(
                f"Çok fazla aşırı aykırı değer: {outliers} ({outlier_pct:.1f}%)"
            )
    ...
    if (series <= 0).any():
        ...
        warnings.warn(
            f"Seride {negative_count} adet sıfır veya negatif değer var. "
...
def detect_data_issues(series: pd.Series) -> dict:
    ...
    if iqr > 0:
        ...
        if outlier_mask.any():
            ...
            issues["warnings"].append(
                f"{len(outlier_indices)} aykırı değer tespit edildi"
            )
    ...
    if len(series) > 12:
        ...
        if large_changes.any():
            ...
            issues["warnings"].append(
                f"{len(shift_indices)} olası seviye değişimi tespit edildi"
            )
    ...
    if len(series) > 24:
        ...
        if vol_changes.any():
            ...
            issues["warnings"].append(
                f"{len(vol_indices)} volatilite değişimi tespit edildi"
            )
