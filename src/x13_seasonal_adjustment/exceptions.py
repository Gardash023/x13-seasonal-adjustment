...
class DataValidationError(X13SeasonalAdjustmentError):
    ...
    def __init__(self, message: str, data_type: Optional[str] = None, **kwargs):
        ...
        super().__init__(
            message, error_code="DATA_VALIDATION", data_type=data_type, **kwargs
        )
...
class TimeSeriesError(X13SeasonalAdjustmentError):
    ...
    def __init__(self, message: str, series_info: Optional[dict] = None, **kwargs):
        ...
        super().__init__(
            message, error_code="TIME_SERIES", series_info=series_info, **kwargs
        )
...
class SeasonalityError(X13SeasonalAdjustmentError):
    ...
    def __init__(
        self, message: str, seasonality_tests: Optional[dict] = None, **kwargs
    ):
        self.seasonality_tests = seasonality_tests or {}
        super().__init__(
            message, error_code="SEASONALITY", tests=seasonality_tests, **kwargs
        )
...
class ARIMAModelError(X13SeasonalAdjustmentError):
    ...
    def __init__(
        self,
        message: str,
        model_order: Optional[tuple] = None,
...
class OutlierDetectionError(X13SeasonalAdjustmentError):
    ...
    def __init__(self, message: str, outlier_types: Optional[list] = None, **kwargs):
        ...
        super().__init__(
            message, error_code="OUTLIER_DETECTION", types=outlier_types, **kwargs
        )
...
class ConfigurationError(X13SeasonalAdjustmentError):
    ...
    def __init__(
        self, message: str, parameter: Optional[str] = None, value: Any = None, **kwargs
    ):
...
class ModelNotFittedError(X13SeasonalAdjustmentError):
    ...
    def __init__(
        self, message: str = "Model must be fitted before calling this method"
    ):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            raise DataValidationError(
                f"Data parsing failed: {str(e)}", data_type="parsing"
            )
        except pd.errors.EmptyDataError as e:
            raise DataValidationError(
                f"Empty data provided: {str(e)}", data_type="empty"
            )
        except KeyError as e:
            raise DataValidationError(
                f"Missing required column or index: {str(e)}", data_type="key_error"
            )
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if "NaN" in str(e) or "inf" in str(e):
                raise DataValidationError(
                    f"Invalid numerical values: {str(e)}", data_type="numerical"
                )
            raise
