...
class X13Logger:
    ...
    def configure_logging(
        ...
        console_formatter = logging.Formatter(
            formats.get(format_type, formats["standard"]), datefmt="%Y-%m-%d %H:%M:%S"
        )
        ...
        if log_to_file:
            ...
            file_formatter = logging.Formatter(
                formats["detailed"], datefmt="%Y-%m-%d %H:%M:%S"
            )
...
def log_execution_time(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.info(
                f"{func.__name__} completed successfully in {execution_time:.3f} seconds"
            )
            return result
