...
class QualityDiagnostics:
    ...
    def evaluate(self, result: SeasonalAdjustmentResult) -> QualityReport:
        ...
        recommendations = self._generate_recommendations(
            m_stats, q_stats, overall_quality
        )
    ...
    def _calculate_m_statistics(
        self, result: SeasonalAdjustmentResult
    ) -> Dict[str, float]:
    ...
    def _calculate_m11(self, seasonal: pd.Series) -> float:
        ...
        try:
            ...
            for month in range(12):  # Monthly data
                ...
                month_values = [
                    seasonal.iloc[i] for i in month_indices if i < len(seasonal)
                ]
    ...
    def _generate_recommendations(
        self, m_stats: Dict[str, float], q_stats: Dict[str, float], overall_quality: str
    ) -> List[str]:
        """Öneriler oluşturur."""
        ...
        if overall_quality in ["Mükemmel", "İyi"]:
            recommendations.append(
                "Seasonal adjustment kalitesi yeterli. Sonuçlar kullanılabilir."
            )
        elif overall_quality == "Orta":
            recommendations.append(
                "Seasonal adjustment kalitesi orta düzeyde. Dikkatli kullanım önerilir."
            )
        else:
            recommendations.append(
                "Seasonal adjustment kalitesi düşük. Model parametrelerini gözden geçirin."
            )

        # Specific M statistic recommendations
        if m_stats.get("M1", 0) > 1.0:
            recommendations.append(
                "M1 yüksek: Irregular component çok büyük. Outlier tespiti yapın."
            )

        if m_stats.get("M7", 0) > 1.0:
            recommendations.append(
                "M7 yüksek: Trend çok volatile. Henderson filter uzunluğunu artırın."
            )

        if m_stats.get("M9", 0) > 1.0:
            recommendations.append(
                "M9 yüksek: Seasonal factors instabil. Seasonal MA filter'ı ayarlayın."
            )

        if m_stats.get("M4", 0) > 1.0:
            recommendations.append(
                "M4 yüksek: Irregular component'te autocorrelation var. ARIMA modelini kontrol edin."
            )

        return recommendations

    def _generate_warnings(
        self, m_stats: Dict[str, float], q_stats: Dict[str, float]
    ) -> List[str]:
        """Uyarılar oluşturur."""
        ...
        for stat_name, value in m_stats.items():
            if np.isinf(value):
                warnings.append(
                    f"{stat_name} hesaplanamadı. Veri yetersizliği olabilir."
                )
            elif value > 5.0:
                warnings.append(
                    f"{stat_name} çok yüksek ({value:.2f}). Kalite sorunu var."
                )

        if q_stats.get("Q", 0) > 3.0:
            warnings.append(
                "Q istatistiği çok yüksek. Seasonal adjustment başarısız olabilir."
            )

        # Check for specific problematic patterns
        if m_stats.get("M1", 0) > 2.0 and m_stats.get("M3", 0) > 2.0:
            warnings.append(
                "Irregular component hem büyük hem instabil. Model uygun değil."
            )
