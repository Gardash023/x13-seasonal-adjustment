...
class SeasonalityTests:
    ...
    def run_all_tests(self, series: pd.Series) -> SeasonalityTestResult:
        ...
        # Birleşik karar ver
        valid_tests = [
            evidence for evidence in seasonal_evidence if evidence is not None
        ]
        ...
        recommendations = self._generate_recommendations(
            test_results, has_seasonality, confidence
        )
    ...
    def x11_seasonality_test(self, series: pd.Series) -> Dict:
        ...
        try:
            ...
            return {"is_seasonal": None, "error": f"X11 test failed: {str(e)}"}
    ...
    def qs_test(self, series: pd.Series) -> Dict:
        ...
        try:
            ...
            if len(seasonal_diff) < self.seasonal_period:
                return {"is_seasonal": None, "error": "Insufficient data for QS test"}

            # Ljung-Box test on seasonal differences
            lags = min(self.seasonal_period, len(seasonal_diff) // 4)

            if lags < 1:
                return {"is_seasonal": None, "error": "Not enough lags for QS test"}
            ...
            return {"is_seasonal": None, "error": f"QS test failed: {str(e)}"}
    ...
    def friedman_test(self, series: pd.Series) -> Dict:
        ...
        try:
            ...
            return {"is_seasonal": None, "error": f"Friedman test failed: {str(e)}"}
    ...
    def autocorrelation_test(self, series: pd.Series) -> Dict:
        ...
        try:
            ...
            autocorr, confint = acf(series, nlags=max_lags, alpha=self.alpha, fft=True)
            ...
            seasonal_acf_sum = sum(
                [abs(autocorr[lag]) for lag in seasonal_lags if lag < len(autocorr)]
            )
    ...
    def combined_seasonality_test(self, series: pd.Series) -> Dict:
        ...
        try:
            ...
            return {"is_seasonal": None, "error": f"Combined test failed: {str(e)}"}
    ...
    def _detect_periodogram_peak(self, series: pd.Series) -> Dict:
        ...
        try:
            ...
            return {"has_seasonal_peak": False, "error": "Periodogram analysis failed"}

    def _generate_recommendations(
        self, test_results: Dict, has_seasonality: bool, confidence: float
    ) -> List[str]:
        ...
        if has_seasonality:
            if confidence > 0.8:
                recommendations.append(
                    "Güçlü mevsimsellik tespit edildi. X13 seasonal adjustment önerilir."
                )
            elif confidence > 0.6:
                recommendations.append(
                    "Orta düzeyde mevsimsellik var. Seasonal adjustment yapılabilir."
                )
            else:
                recommendations.append(
                    "Zayıf mevsimsellik tespit edildi. Dikkatli seasonal adjustment gerekli."
                )

            # Specific test recommendations
            if "x11_test" in test_results and test_results["x11_test"].get(
                "is_seasonal"
            ):
                recommendations.append(
                    "X11 F-test pozitif: Additive seasonal decomposition uygun."
                )

            if "autocorrelation" in test_results:
                autocorr_result = test_results["autocorrelation"]
                if (
                    autocorr_result.get("is_seasonal")
                    and autocorr_result.get("seasonal_acf_sum", 0) > 0.5
                ):
                    recommendations.append(
                        "Güçlü seasonal autocorrelation: SARIMA modeli önerilir."
                    )
            ...
            if confidence < 0.2:
                recommendations.append(
                    "Seasonal adjustment gerekli değil. Trend-cycle analysis yeterli olabilir."
                )
            else:
                recommendations.append(
                    "Borderline durum: Ek testler veya uzman değerlendirmesi önerilir."
                )

        # Data quality recommendations
        if len([r for r in test_results.values() if "error" in r]) > 0:
            recommendations.append(
                "Bazı testler başarısız oldu. Veri kalitesini kontrol edin."
            )
