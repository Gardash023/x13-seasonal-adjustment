...
class SeasonalDecomposition:
    ...
    def _extend_series(self, series: pd.Series) -> pd.Series:
        ...
        if len(forecast) != self.forecast_maxlead:
            ...
            if len(forecast) < self.forecast_maxlead:
                ...
                additional_values = np.repeat(
                    last_value, self.forecast_maxlead - len(forecast)
                )
        ...
        if len(backcast) != self.backcast_maxlead:
            if len(backcast) < self.backcast_maxlead:
                ...
                additional_values = np.repeat(
                    first_value, self.backcast_maxlead - len(backcast)
                )
    ...
    def _estimate_seasonal_factors(
        self, seasonal_irregular: pd.Series, mode: str, iteration: int = 1
    ) -> pd.Series:
                ...
                if is_nan:
                    ...
                    if not pd.isna(period_means.iloc[period]):
                        ...
                        if not pd.isna(overall_mean):
                            ...
                            seasonal_factors.iloc[i] = (
                                1.0 if mode == "multiplicative" else 0.0
                            )
        ...
        if mode == "multiplicative":
            ...
            period_groups = np.arange(len(seasonal_factors)) % self.seasonal_period
            mean_factor = (
                seasonal_factors.groupby(period_groups).mean().sum()
    ...
    def _henderson_filter(self, series: pd.Series) -> pd.Series:
        ...
        filtered = series.rolling(window=n, center=True).apply(
            apply_henderson, raw=True
        )
    ...
    def _henderson_weights(self, n: int) -> np.ndarray:
        ...
        if n == 13:
            ...
            weights = np.array(
                [-0.041, -0.010, 0.119, 0.267, 0.330, 0.267, 0.119, -0.010, -0.041]
            )
        else:
