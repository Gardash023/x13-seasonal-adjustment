...
from ..logging_config import LoggingContextManager, X13Logger, log_execution_time
...
class X13SeasonalAdjustment(BaseEstimator, TransformerMixin):
        ...
        **kwargs,
        ...
        try:
            ...
            if enable_logging:
                ...
                self.logger.debug(
                    f"Configuration: freq={freq}, transform={transform}, "
            ...
            if enable_logging:
                self.logger.error(
                    f"Failed to initialize X13SeasonalAdjustment: {str(e)}"
                )
        ...
        try:
            ...
            if not isinstance(self.freq, str):
                raise ConfigurationError(
                    "freq must be a string", parameter="freq", value=self.freq
                )
            ...
            if self.arima_order != "auto":
                if (
                    not isinstance(self.arima_order, (tuple, list))
            ...
            if self.seasonal_arima_order != "auto":
                if (
                    not isinstance(self.seasonal_arima_order, (tuple, list))
                    or len(self.seasonal_arima_order) != 3
                ):
                    raise ConfigurationError(
    ...
    def _apply_transform(
        self, series: pd.Series, transform_type: str
    ...
    def fit(self, X: Union[pd.Series, pd.DataFrame], y=None) -> "X13SeasonalAdjustment":
        ...
        try:
            if self.enable_logging:
                ...
                self.logger.debug(
                    f"Input data shape: {X.shape if hasattr(X, 'shape') else 'N/A'}"
                )
            ...
            try:
                preprocessed_series, self._preprocessing_info = preprocess_series(
                    series, handle_missing=True, detect_outliers=self.outlier_detection
                )

                if self.enable_logging and self._preprocessing_info.get("outliers"):
                    outlier_count = len(self._preprocessing_info["outliers"])
                    self.logger.info(
                        f"Detected {outlier_count} outliers during preprocessing"
                    )
            ...
            try:
                ...
                seasonality_result = self._seasonality_tester.run_all_tests(
                    preprocessed_series
                )

                if self.enable_logging:
                    self.logger.info(
                        f"Seasonality detected: {seasonality_result.has_seasonality}"
                    )
                    if hasattr(seasonality_result, "test_results"):
                        for (
                            test_name,
                            result,
                        ) in seasonality_result.test_results.items():
                            self.logger.debug(f"  {test_name}: {result}")

                if not seasonality_result.has_seasonality:
                    warning_msg = (
                        "No clear seasonality detected. Results may be unreliable."
                    )
                    if self.enable_logging:
            ...
            try:
                ...
                transformed_series, transform_info = self._apply_transform(
                    preprocessed_series, transform_type
                )
                ...
                if self.enable_logging:
                    ...
                    if transform_info.get("shift"):
                        self.logger.debug(
                            f"Applied data shift: {transform_info['shift']}"
                        )
            ...
            try:
                ...
                if self.enable_logging:
                    self.logger.info(
                        f"ARIMA model fitted: {self._arima_model.order_} x {self._arima_model.seasonal_order_}"
                    )
                    self.logger.debug(
                        f"Model AIC: {self._arima_model.aic_:.3f}, BIC: {self._arima_model.bic_:.3f}"
                    )
            ...
            try:
                ...
                raise DecompositionError(
                    f"Seasonal decomposer initialization failed: {str(e)}"
                ) from e
            ...
            if self.enable_logging:
                self.logger.info(
                    f"X13 model fitting completed successfully in {total_time:.3f} seconds"
                )
                self.logger.debug(
                    "Timing breakdown: "
                    + ", ".join(
                        [
                            f"{k}: {v:.3f}s"
                            for k, v in self._timing_info.items()
                            if k != "total_fit_time"
                        ]
                    )
                )
    ...
    def transform(self, X: Union[pd.Series, pd.DataFrame]) -> SeasonalAdjustmentResult:
        ...
        try:
            ...
            try:
                ...
                if self.enable_logging:
                    self.logger.debug(
                        f"Validated input series with {len(series)} observations"
                    )
            except Exception as e:
            ...
            try:
                ...
                transformed_series, _ = self._apply_transform(
                    preprocessed_series, transform_info["type"]
                )
            except Exception as e:
                raise TransformationError(
                    f"Data transformation failed: {str(e)}"
            ...
            try:
                decomposition_result = self._seasonal_decomposer.decompose(
                    transformed_series
                )
                ...
                if self.enable_logging:
                    ...
                    self.logger.info(
                        f"Seasonal decomposition completed using {mode} mode"
                    )
            ...
            try:
                seasonally_adjusted = self._reverse_transform(
                    decomposition_result["seasonally_adjusted"], transform_info
                )
                trend = self._reverse_transform(
                    decomposition_result["trend"], transform_info
                )
                ...
                raise TransformationError(
                    f"Scale transformation failed: {str(e)}"
                ) from e
            ...
            if self.enable_logging:
                self.logger.info(
                    f"Seasonal adjustment completed in {total_time:.3f} seconds"
                )
                self.logger.debug(
                    f"Timing: preprocessing={preprocessing_time:.3f}s, "
    ...
    def _calculate_seasonality_strength(
        self, original: pd.Series, seasonal: pd.Series
    ) -> float:
