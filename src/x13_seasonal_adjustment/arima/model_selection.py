...
class ARIMAModelSelector:
    ...
    def _cross_validate_model(
        self, fitted_model, series: pd.Series, X: Optional[pd.DataFrame]
    ) -> Dict[str, float]:
        ...
        try:
            ...
            for fold in range(self.cv_folds):
                ...
                train_series = series.iloc[
                    : test_start + int(0.8 * (test_end - test_start))
                ]
                test_series = series.iloc[
                    test_start + int(0.8 * (test_end - test_start)) : test_end
                ]
                ...
                try:
                    ...
                    forecast = cv_fitted.forecast(
                        ...
                        exog=(
                            X.iloc[
                                len(train_series) : len(train_series) + len(test_series)
                            ]
                            if X is not None
                            else None
                        ),
                    ...
                    cv_errors.append({"mae": mae, "mse": mse, "rmse": rmse})
    ...
    def summary_report(self) -> str:
        ...
        if self.best_model:
            ...
            report.append(
                f"Model: ARIMA{self.best_model['order']}x{self.best_model['seasonal_order']}"
            )
