...
class AutoARIMA:
    ...
    def _stepwise_search(
        self, y: pd.Series, d: int, D: int, X: Optional[pd.DataFrame]
    ) -> Dict[str, Any]:
        ...
        while improved:
            improved = False
            candidates = self._get_neighbor_models(
                current_order, current_seasonal_order
            )
            ...
            for order, seasonal_order in candidates:
                if sum(order) + sum(seasonal_order) <= self.max_order:
                    ...
                    if (
                        model_result
                        and model_result[self.information_criterion] < best_ic
                    ):
                        best_ic = model_result[self.information_criterion]
                        best_model = model_result
                        current_order = order
    ...
    def forecast(
        self, steps: int = 1, X: Optional[pd.DataFrame] = None, alpha: float = 0.05
    ...
    def diagnostic_tests(self) -> Dict[str, Any]:
        ...
        try:
            ljung_box = acorr_ljungbox(
                resid, lags=min(10, len(resid) // 4), return_df=True
            )
